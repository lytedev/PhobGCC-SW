...
enum WhichTrigger { LTRIGGER, RTRIGGER };

enum Increase { INCREASE, DECREASE };

enum WhichAxis { XAXIS, YAXIS };

enum WhichStick { ASTICK, CSTICK };

enum HardReset { HARD, SOFT, FACTORY };

enum NotchStatus { TERT_INACTIVE, TERT_ACTIVE, SECONDARY, CARDINAL };
...
struct FilterGains {
  ...
  float
      xVelDecay; // 0.1 default for 1.2ms timesteps, larger for bigger timesteps
  float yVelDecay;
  // how much the current position disagreement impacts the filtered velocity.
  // Probably don't touch this.
  float xVelPosFactor; // 0.01 default for 1.2ms timesteps, larger for bigger
                       // timesteps
  float yVelPosFactor;
  // how much to ignore filtered velocity when computing the new stick position.
  // DO CHANGE THIS
  // Higher gives shorter rise times and slower fall times (more pode, less
  // snapback)
  float xVelDamp; // 0.125 default for 1.2ms timesteps, smaller for bigger
                  // timesteps
  float yVelDamp;
  ...
  float
      accelThresh; // 5 default for 1.2ms timesteps, larger for bigger timesteps
